...
/*   Updated: 2023/02/28 15:22:07 by itovar-n         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_totroube(t_list **lst_a, t_listc **sol)
{
	...
	count = 0;
	while (ft_lsttrouble(*lst_a) > 0 && ft_lstsorted(*lst_a) == 0
		&& ((*lst_a)->content >= ft_lstlast(*lst_a)->content || count == 0))
	{
		a = ft_lsttrouble(*lst_a);
		if (ft_lstsorted(*lst_a) == 1)
			ft_ps_exit(lst_a, sol);
		ft_tocount_a(lst_a, ft_lsttrouble(*lst_a) - 1, sol);
		if ((*lst_a) && (*lst_a)->next && (*lst_a)->next->next
			&& (*lst_a)->content > (*lst_a)->next->content
			&& (*lst_a)->next->content > ft_lstlast(*lst_a)->content
			&& ((*lst_a)->content < (*lst_a)->next->next->content
				|| ft_lstsorted_s((*lst_a)->next->next) == 1))
			sa(lst_a, sol);
		else if (ft_strncmp(ft_llastc(*sol)->content, "ra\n", 3) == 0 && a >= 2)
			ra(lst_a, sol);
		else if (ft_strncmp(ft_llastc(*sol)->content, "rra\n", 4) == 0 && a >= 2)
			rra(lst_a, sol);
		count++;
	}
	if (ft_lstsorted(*lst_a) == 1)
